        ...
        <Card className="group overflow-hidden relative h-full">
            <Link className='block' href={`/projects/${project.slug}`}>
                ...
                <div className='h-full px-6 py-4'>
                    ...
                    <h4 className='h4 mb-1'>{project.title.length > 35 ? project.title.substr(0, 35) + "..." : project.title}</h4>
                    <p className='text-muted-foreground text-lg'>{project.resume.length > 100 ? project.resume.substr(0, 100) + "..." : project.resume}</p>
                </div>
            </Link>
        </Card>
    );
